...
// script.src = 'https://fastly.jsdelivr.net/npm/echarts@5.4.2/dist/echarts.min.js';
...
const ContentAndUrl = { '数据总览': 1, '统计任务': 2, '首页': 3, '任务统计结果内容': 1000, '单车统计结果':2000 };
...
// 创建一个对象来存储已缓存的页面内容
const cache = {};
// 存储任务后台数据库编号与的需要渲染成Echarts的配置项的对应关系
var TaskNumEcharts = 0;
// 存储上一个打开的Task的数据库编号以进行后续的重新渲染Echarts
var TempTaskNum = {};
let windowNum = 0;
let tempurl = 0;
let tempcontent = 0;
    ...
    if (existingWindow) {
        if (activeWindow === existingWindow) {
            ...
            if (url === 2) {
                ...
                console.log('存储的EchartsOption配置为：',options);
                createDualAxisChartSpeedMile('EchartsPointsPicture',options[url],TempTaskNum[url]);
            }
...
function createWindowContentElement(content, windowId, url) {
    ...
    deleteButton.addEventListener('click', (event) => {
        ...
        closeWindow(windowId, url, content);
    });
...
function closeWindow(windowId, url, content) {
...
function loadContent(url, containerId, content) {
    ...
    if (container) {
        ...
        if (cache[url] && cache[url][content]) {
            ...
            if(url >= 1000){
                console.log('用loadContent函数，存储的EchartsOption配置为：',options[url]);
                createDualAxisChartSpeedMile('EchartsPointsPicture', options[url],TempTaskNum[url]);
            ...
            // if (url >= 2000){
            //     ShowCarDataByPoint(TempVin[url],TempTaskNum[url]);
            // }
...
function showTaskResult(fb, db, TaskNum, TaskShowSortNum) {
    var fuelCheckbox = document.getElementById(fb);
    var dpfCheckbox = document.getElementById(db);
    const data = {
        TaskNum: TaskNum,
    };
    var content = '任务' + TaskShowSortNum + '统计结果内容';
    // content = '任务统计结果内容';
    const existingWindow = openWindows[content];
    //如果该任务统计窗口目前已经处于打开状态，则只进行打开操作而无需再查询后台更新内容，按照原有逻辑，会导致任务结果统计页面重复性再次多出一遍原有内容
    if (existingWindow) {
        alert('同名任务统计窗口只能同时开启一个，请通过鼠标点击切换');
        
        openWindow(content, ContentAndUrl[content]);
        return;
    }
    ...
    fetch('/ShowTaskResult', {
        ...
        .then(data => {
            ...
            let maxNumber = -1; // 初始化一个较小的值，确保所有数字都在范围之外
            let maxName = '';

            if(ContentAndUrl[content]){
                maxNumber = ContentAndUrl[content];

            }

            else{
                for (const name in ContentAndUrl) {
                const number = ContentAndUrl[name];
                if (number > 999 && number < 2000 && number > maxNumber) {
                    maxNumber = number;
                ...
                if (maxName !== '') {
                    ContentAndUrl[maxName] = maxNumber;
                    HTMLContents[maxNumber] = `<div id="Forms">
                    </div>
                    <div id="EchartsPointsPicture"></div>
                    <div id="Images"></div>`;
                }
           }
            openWindow(content, maxNumber);

            //传递参数，更新任务统计结果页面的内容
            // TempTaskNum[maxNumber] = TaskNum;
            // console.log('TempTaskNum内容为：',TempTaskNum);
            updateTaskResultContent(TaskNum, TaskShowSortNum,ContentAndUrl[content]);
...
//更新任务统计结果页面的内容(表格+Echarts+图片)
function updateTaskResultContent(TaskNum, TaskShowSortNum,url) {
    var content = '任务' + TaskShowSortNum + '统计结果内容';
    const data = {
        TaskNum: TaskNum,
    };
    ...
    fetch('/UpdateTaskResultContent', {
        ...
        .then(data => {
            ...
            var option = {
                // Your option configuration here...
                ...
                series: [
                    {
                        ...
                        data: [
                            [100, 3000,'FAWXCECTID6040699'], [38, 1400,'FAWXCECTID6040699']
                        ]
                    },
                    {
                        type: 'scatter',
                        ...
                        data: [
                            [5, 680,'FAWXCECTID6040699'], [80, 1140,'FAWXCECTID6040699'], [32, 222,'FAWXCECTID6040699'], [50, 920,'FAWXCECTID6040699'], [20, 442,'FAWXCECTID6042711'], [100, 3000,'FAWXCECTID6042711'], [38, 1400,'FAWXCECTID6042711'], [60, 2000,'FAWXCECTID6042711'], [15, 800,'FAWXCECTID6042711'], [28, 2500,'FAWXCECTID6042711']
                        ]
                    },
                    {
                        type: 'scatter',
                ...
                //注意：tooltip的位置！这里经尝试需要放在series之外与之同级（天气预报风向实例中可以放在内部，原理未知），之前将其放在了effectscatter的下一级被卡住了许久
      tooltip: {
        // 设置提示框属性
        trigger: 'item', // 触发条件为鼠标悬浮在数据项上
        formatter: function (params) {
          ...
          return;
                ...
                visualMap: {
                    ...
                    dimension: 0,
                    pieces: [
                        //gt表示大于，lt表示小于，加e表示加个等于
                        {
                            min: 70,
                            color: 'rgba(249, 64, 3, 1)',
                            label: '高速路         '
                        },
                        {
                            max: 70,
                            min: 30,
                            color: 'rgba(249, 159, 3, 1)',
                            label: '市郊路      '
                        },
                        {
                            max: 30,
            ...
            TaskNumEcharts = option;
            options[url] = option;
            TempTaskNum[url] = TaskNum;
      
            createDualAxisChartSpeedMile('EchartsPointsPicture', options[url], TaskNum);
            ...
            for (let i = 1; i <= taskCount; i++) {
                ...
                <div class="Task" data-TaskNum="${task.TaskNum}">
                    ...
                    <span class="TaskChoice">
                        ...
                        <button class="ShowResult" onclick="showTaskResult('Fuelcheckbox${i}', 'DPFcheckbox${i}', '${task.TaskNum}','${i}')">查看统计结果</button>
...
function createDualAxisChartSpeedMile(containerId, option, TaskNum) {
    
    var dom = document.getElementById(containerId);
    //检查是否已经存在一个echarts实例 
    var existingChart = echarts.getInstanceByDom(dom); 
    // 如果存在，则销毁它
    //注意！！！原来我的处理方式是将dom元素通过innerHTML=``的方法试图重置重新渲染
    //事实证明是不行的，得通过dispose方法将其彻底销毁才能重新渲染出来，否则一切换页面就会变成一片空白
    //此处卡了大半天，GPT解决不了，需要问bingai 
    if (existingChart) { 
    ...
    myChart.on('click', function (params) {
        if (params.componentType === 'series' && (params.seriesType === 'effectScatter' || params.seriesType === 'scatter')) {
            ...
            //如果已经小窗名和页面url内容对应集已经记录了此VIN码，则要打开的窗口content为params.data[2],url为TempVin中属性名为此VIN码的属性值
            if(params.data[2] in ContentAndUrl){
                maxName =  params.data[2];
                maxNumber = TempVin[maxName];
            }
            else{
                for (const name in ContentAndUrl) {
                    const number = ContentAndUrl[name];
                    if (number > 1999 && number > maxNumber) {
                        maxNumber = number;
                ...
                if (maxName !== '') {
                    // TempVin[maxNumber] = params.data[2];
                    TempVin[params.data[2]] = maxNumber;
            ...
            openWindow(params.data[2] + '',maxNumber);
            ShowCarDataByPoint(params.data[2],TaskNum);
            
            var existingWindow=openWindows[params.data[2]];
...
function ShowCarDataByPoint(Vin,TaskNum) {
    const data = {
        Vin: Vin,
        TaskNum:TaskNum,
    };
    // 使用AJAX请求将数据发送给FLASK后台
    fetch('/UpdateAloneCarImage', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Access-Control-Allow-Origin': '*'
...
// 假设之前已经初始化并渲染了一个 Echarts 图表到名为 'EchartsPointsPicture' 的容器中
// 在页面切换后，重新显示图表
function showEchartsAgain() {
    var containerId = 'EchartsPointsPicture';

    // 获取现有的 Echarts 实例
    var existingChart = echarts.getInstanceByDom(document.getElementById(containerId));

    if (existingChart) {
        // 修改图表容器的样式，使其可见
        document.getElementById(containerId).style.display = 'block';

        // 调用图表实例的 resize 方法，重新渲染图表
