...
const ContentAndUrl = { '数据总览': 1, '统计任务': 2, '首页': 3, '任务统计结果内容': 1000, '单车统计结果':1001 };
    ...
    if (existingWindow) {
        if (activeWindow === existingWindow) {
            ...
            if (url === 2) {
                ...
                console.log('存储的EchartsOption配置为：',TaskNumEcharts);
                createDualAxisChartSpeedMile('EchartsPointsPicture', TaskNumEcharts);
            }
...
function loadContent(url, containerId, content) {
    ...
    if (container) {
        ...
        if (cache[url] && cache[url][content]) {
            ...
            if(url === 1000){
                console.log('用loadContent函数，存储的EchartsOption配置为：',TaskNumEcharts);
                createDualAxisChartSpeedMile('EchartsPointsPicture', TaskNumEcharts);
            ...
            if (url === 1001){
                ShowCarDataByPoint(TempVin,TempTaskNum);
            }
...
function showTaskResult(fb, db, TaskNum, TaskShowSortNum) {
    ...
    if (existingWindow) {
        alert('任务统计窗口只能同时开启一个');
        openWindow(content, 1000);
        return;
    }
    ...
    fetch('/ShowTaskResult', {
        ...
        .then(data => {
            ...
            //传递参数，更新任务统计结果页面的内容
            TempTaskNum = TaskNum;
            updateTaskResultContent(TaskNum, TaskShowSortNum);
...
//更新任务统计结果页面的内容
function updateTaskResultContent(TaskNum, TaskShowSortNum) {
...
function createDualAxisChartSpeedMile(containerId, option) {
    ...
    myChart.on('click', function (params) {
        if (params.componentType === 'series' && (params.seriesType === 'effectScatter' || params.seriesType === 'scatter')) {
            ...
            // window.open(url);
            TempVin = params.data[2];

            openWindow(params.data[2],1001);
            var existingWindow=openWindows[params.data[2]];
            if(!existingWindow){
                updateCarDataPage(params.data[2]);
            }
        }
    });
